document.addEventListener("DOMContentLoaded", async function () {
    ...
    firebase.auth().onAuthStateChanged((user) => {
        ...
        advisoriesRefActive.on('value', snapshot => {
            ...
            activeAdvisories = advisoriesData
                ? Object.entries(advisoriesData)
                    .map(([id, advisory]) => ({ id, ...advisory }))
                    .filter(a => a.advisory_status === 'Active' && a.creator === currentUserUID)
                : [];
    ...
    async function fetchUserName(uid) {
        try {
            ...
            if (!userSnap.exists()) return "Unknown Station";
            ...
            const stationSnap = await firebase.database().ref(`emergency_responder_station/${stationId}`).once("value");
    
            if (!stationSnap.exists()) return "Unknown Station";
    
            const stationData = stationSnap.val();
            return stationData.station_name || "Unknown Station";
    ...
    async function displayAdvisories(advisories, searchQuery = "") {
        ...
        if (filteredAdvisories.length === 0) {
            tableBody.innerHTML = `<tr><td colspan="4" class="text-center text-danger">No Active Advisories.</td></tr>`;
            return;
        }
    ...
    function openViewModal(advisoryId) {
        ...
        document.getElementById("viewModalCreator").textContent = "Loading...";
        ...
        // ✅ Fetch the creator's station name
        firebase.database().ref(`users/${advisory.creator}`).once("value")
        .then(userSnapshot => {
            const userData = userSnapshot.val();
    
            if (userData && userData.station_id) {
                return firebase.database().ref(`emergency_responder_station/${userData.station_id}`).once("value");
            } else {
                throw new Error("No station assigned");
            }
        })
        .then(stationSnapshot => {
            const stationData = stationSnapshot.val();
            if (stationData) {
                document.getElementById("viewModalCreator").textContent = stationData.station_name;
            } else {
                document.getElementById("viewModalCreator").textContent = "Unknown Station";
            }
        })
        .catch(error => {
