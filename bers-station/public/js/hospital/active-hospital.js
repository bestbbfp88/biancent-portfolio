document.addEventListener("DOMContentLoaded", function () {
    ...
    async function openUpdateHospitalModal(hospitalId) {
        try {
            ...
            document.getElementById("updateHospitalLat").value = hospitalData.latitude;
            document.getElementById("updateHospitalLng").value = hospitalData.longitude;
    ...
    // Google Places API: Auto-compute Latitude & Longitude
    function initAutocomplete() {
        const input = document.getElementById("updateHospitalAddress");
        const autocomplete = new google.maps.places.Autocomplete(input);

        autocomplete.addListener("place_changed", function () {
            const place = autocomplete.getPlace();
            if (place.geometry) {
                document.getElementById("updateHospitalLat").value = place.geometry.location.lat();
                document.getElementById("updateHospitalLng").value = place.geometry.location.lng();
            }
        });
    }
    ...
    document.getElementById("updateHospitalForm").addEventListener("submit", async function (event) {
        ...
        const updatedLat = document.getElementById("updateHospitalLat").value.trim();
        const updatedLng = document.getElementById("updateHospitalLng").value.trim();
        const updatedContacts = document.querySelectorAll(".update-contact-number");

        if (!updatedName || !updatedAddress || !updatedLat || !updatedLng || updatedContacts.length === 0) {
        ...
        try {
            ...
            await hospitalRef.update({
                ...
                latitude: parseFloat(updatedLat),
                longitude: parseFloat(updatedLng),
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function initAutocomplete() {
        ...
        autocomplete = new google.maps.places.Autocomplete(addressInput, {
            ...
            componentRestrictions: { country: "PH" } // Set to your country code if needed
        });

        autocomplete.addListener("place_changed", function () {
            const place = autocomplete.getPlace();
            if (!place.geometry) {
                console.error("❌ Error: No geometry found for this location.");
                return;
            }
            ...
            console.log("✅ Auto-computed location: ", {
