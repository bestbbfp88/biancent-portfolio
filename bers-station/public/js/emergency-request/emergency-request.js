document.addEventListener("DOMContentLoaded", () => {
  firebase.auth().onAuthStateChanged(async (user) => {
    ...
    // 🔁 Real-time listener for emergencies assigned to signed-in user
    ...
    emergencyRef.on("value", (snapshot) => {
      ...
      if (snapshot.exists()) {
        snapshot.forEach((child) => {
          const data = child.val();
          if (data.assign_station === signedInUID && data.date_time && data.user_ID) {
            allEmergencies.push({
              ...data,
              callerName: userMap[data.user_ID] || "Unknown"
            });
          }
        });

        populateDropdowns(allEmergencies);
        applyFilters(); // Apply current filters after update
      }
    });
  });
});
  ...
  function updateTimestamp() {
    ...
    document.getElementById('printDetailsBtn').addEventListener('click', () => {
      ...
      document.getElementById('downloadPdfBtn').addEventListener('click', () => {
        ...
        const element = document.querySelector('#viewEmergencyModal .modal-body');
        const opt = {
          margin:       0.5,
          filename:     'emergency-details.pdf',
